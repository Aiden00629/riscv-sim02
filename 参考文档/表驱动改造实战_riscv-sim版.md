...
## 八、实际完成记录（2026-09-06，与方案的出入）

第 1~4 步已由组长完成（亲手写代码 + 逐坑排查），与本文档方案的出入记录如下：

| 出入点 | 方案 | 实际 | 影响 |
|---|---|---|---|
| 拆括号函数名 | `parseMemOperand` | `encodehelp` | 名字不达意（它不编码），功能相同，可后续改名 |
| encodeM | 删除、复用 encodeI | **保留**（与 encodeI 重复） | 功能正确，只是两份相同代码；删 if 链时顺手清理 |
| encodeR 的 u32 位置 | `u32(desc.funct7) << 25` | `u32(desc.funct7 << 25)`（先移位后转换） | 当前 funct7 最大 0x20 安全；int 移位溢出是 UB，严谨应改 |
| M 分支参数 | `encodeI(desc, rd, rs1, imm)` | `encodeM(desc, rd, rs1, imm)` | 同上，重复函数 |

**实际踩过的坑**（详见踩坑记录 16~18 号）：encodeB 漏 `>>1`（B 型 imm[0] 不存在）、M/S 分支没拆括号（`regNum("0(x2)")` 崩溃）、`== "I"` 双引号编译错。

**教学收获**（组长视角）：表驱动改造是"提炼 + 对照"的过程——每个编码器都从旧 if 块逐位抄出、把写死的数字换成 `desc.` 成员；新旧路径并存跑测试对拍，是重构安全的底气。
